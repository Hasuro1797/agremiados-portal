      ...
      <div className="relative aspect-video bg-gray-100 overflow-hidden">
        {post.coverImage ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-primary/20 via-primary/10 to-primary/5 flex items-center justify-center">
            <Newspaper className="size-10 text-primary/30" />
          </div>
        )}
        {post.isPinned && (
      ...
      <div className="flex flex-col flex-1 p-5 gap-3">
        <div className="flex flex-wrap gap-1.5">
          {post.tags?.slice(0, 2).map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 bg-primary/10 text-primary rounded-full text-[10px] font-semibold uppercase tracking-wide"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="pt-3 border-t border-gray-100">
          <Link
            href={routes.news.detail(post.id)}
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
          <nav className="flex items-center gap-1.5 text-xs text-gray-400 mb-3">
            ...
            <span>/</span>
            <span className="text-gray-700 font-medium">Noticias</span>
          </nav>
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">
                Las últimas noticias y anuncios de nuestra organización.
              </p>
            </div>
          </div>
        </div>
        ...
        {!loading && !error && posts.length === 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center justify-center gap-3 py-24">
            <div className="rounded-2xl bg-gray-50 p-5">
              <Newspaper className="size-9 text-gray-300" />
            </div>
              ...
              <div className="flex items-center justify-center gap-2 mt-10">
                ...
                {getPageNumbers(page, totalPages).map((p, i) =>
                  p === "..." ? (
                    <span
                      key={`ellipsis-${i}`}
                      className="size-8 flex items-center justify-center text-gray-400 text-sm"
                    >
                      …
                    </span>
                  ) : (
