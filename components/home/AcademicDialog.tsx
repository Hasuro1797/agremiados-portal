...
"use client";
import { GENERATE_ACADEMIC_ACTIVITY } from "@/graphql/mutation/academic.mutation";
import { GET_ALL_ACADEMIC_ACTIVITIES } from "@/graphql/query/acedemic.query";
...
export default function AcademicDialog() {
  ...
  const {
    ...
    error: acedemicErrors,
    refetch: refecthAcademicActivities,
  } = useQuery(GET_ALL_ACADEMIC_ACTIVITIES, { fetchPolicy: "no-cache" });

  const [generateAttendace, { loading: attendaceLoading }] = useMutation(
    GENERATE_ACADEMIC_ACTIVITY,
    {
      onError: (error) => {
  ...
  const handleGenerateAttendace = async (id: number) => {
    try {
      await generateAttendace({ variables: { academicId: id } });
    } catch (error) {
      console.error("Error al generar la asistencia", error);
    }
  };

  const handlePay = (idEvent: number) => {
    window.localStorage.setItem("productId", idEvent.toString());
    router.push(`${routes.checkout}/academicEvents`);
  };

  const events: Event[] = get(
    academicActivities,
    "geAcademicActivitiesFromWebsite",
    [],
  );
                ...
                <ScrollArea className="flex-1 min-h-0">
                  <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {events.map((event) => {
                      ...
                      const imageUrl = get(event, "media.[0].url", "");
                      const imageAlt = get(event, "media.[0].alt", event.title);
                      ...
                      return (
                        ...
                        >
                          <div className="relative h-48 overflow-hidden">
                            ...
                            <span className="absolute bottom-4 left-4 bg-[#1a1a3e] text-white text-[10px] font-semibold px-3 py-1 rounded-full uppercase tracking-widest">
                              Evento Académico
                            </span>
                            {discount && (
                          ...
                          <CardContent className="flex flex-col flex-1 p-5 h-[calc(100%-12rem)]">
                            ...
                            <div className="mt-auto space-y-3">
                              {event.date && (
                                <div className="flex text-xs text-slate-500 flex-col gap-0.5">
